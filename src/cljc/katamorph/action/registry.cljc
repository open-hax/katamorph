...
(defn compose
  "Compose portable action registries without silently replacing an action id."
  [& registries]
  (let [errors (vec (mapcat #(-> % validate :errors) registries))
        duplicate-ids (conflicts registries)]
    (cond
      (seq errors)
      {:ok false :reason :invalid-action-registry :errors errors}

      (seq duplicate-ids)
      {:ok false :reason :action-id-conflict :conflicts duplicate-ids}
